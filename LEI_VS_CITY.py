...
lineup_CITY=pd.DataFrame.from_dict(dict_CITY)
print(lineup_LEI.to_markdown())
print(lineup_CITY.to_markdown())
...
players_CITY = {}
for i in range(len(lineup_CITY)):
    key = lineup_CITY.player[i]['name']
    val = lineup_CITY.jersey_number[i]
    players_CITY[key] = str(val)
print("\n",players_CITY)

#Events for LEI & CITYTED

events_pn = LEI_CITY_events[['minute', 'second', 'team', 'type', 'location', 'pass_end_location', 'pass_outcome', 'player','pass_length']]
#print(LEI_CITY_events.columns)
events_LEI = events_pn[events_pn['team'] == 'Leicester City']
events_CITY = events_pn[events_pn['team'] == 'Manchester City FC']

events_pn_CITY = events_CITY[events_CITY['type'] == 'Pass']
...
events_pn_CITY['pass_maker'] = events_pn_CITY['player']
events_pn_CITY['pass_receiver'] = events_pn_CITY['player'].shift(-1)

print(events_pn_LEI.head(10).to_markdown())
print(events_pn_CITY.head(10).to_markdown())

events_pn_LEI2=events_pn_LEI
events_pn_CITY = events_pn_CITY[events_pn_CITY['pass_outcome'].isnull() == True].reset_index()
events_pn_LEI= events_pn_LEI[events_pn_LEI['pass_outcome'].isnull() == True].reset_index()
print(events_pn_LEI.head(10).to_markdown())

#Substitutes
substitution_CITY = events_CITY[events_CITY['type'] == 'Substitution']
...
substitution_CITY_minute = np.min(substitution_CITY['minute'])
substitution_CITY_minute_data = substitution_CITY[substitution_CITY['minute'] == substitution_CITY_minute]
substitution_CITY_second = np.min(substitution_CITY_minute_data['second'])
print("minute =", substitution_CITY_minute, "second =",  substitution_CITY_second)

events_pn_CITY = events_pn_CITY[(events_pn_CITY['minute'] <= substitution_CITY_minute)]
...
plt.title("Adjacency matrix for Leicester vs City pass network")
plt.show()
goal()
player_degree()
