...
sourceFile = open('LEI_V_STO.txt', 'w')
sourceFile2=open('LEI_V_STO_THROUGH.txt','w')
print('Leicester VS Stoke\n--------------------------------',file=sourceFile)

#print(match.to_markdown())

LEI_STO_events=sb.events(match_id=3754341)
for i in range(380):
    if match['match_id'][i] == 3754341:
        print(match['home_team'][i], ':', match['home_score'][i], '-', match['away_team'][i], ':', match['away_score'][i],file=sourceFile)

print(LEI_STO_events.to_markdown(),file=sourceFile2)
#LEI_STO_events.to_csv('TOT_STOKE.csv',index=False)

tact = LEI_STO_events[LEI_STO_events['tactics'].isnull() == False]
tact = tact[['tactics', 'team', 'type']]
print(tact.to_markdown())

tact = tact[tact['type'] == 'Starting XI']
tact_LEI = tact[tact['team'] == 'Leicester City']
tact_STO = tact[tact['team'] == 'Stoke City']
tact_LEI = tact_LEI['tactics']
tact_STO = tact_STO['tactics']
print(tact_LEI.to_markdown())
print(tact_STO.to_markdown())

dict_LEI = tact_LEI[0]['lineup']
dict_STO = tact_STO[1]['lineup']

lineup_LEI = pd.DataFrame.from_dict(dict_LEI)
lineup_STO=pd.DataFrame.from_dict(dict_STO)
print(lineup_LEI.to_markdown())
print(lineup_STO.to_markdown())
...
players_STO = {}
for i in range(len(lineup_STO)):
    key = lineup_STO.player[i]['name']
    val = lineup_STO.jersey_number[i]
    players_STO[key] = str(val)
print("\n",players_STO)

#Events for LEI & STOKE

events_pn = LEI_STO_events[['minute', 'second', 'team', 'type', 'location', 'pass_end_location', 'pass_outcome', 'player','pass_length','play_pattern','pass_through_ball']]
#print(LEI_STO_events.columns)
events_LEI = events_pn[events_pn['team'] == 'Leicester City']
events_STO = events_pn[events_pn['team'] == 'Stoke City']

events_pn_STO = events_STO[events_STO['type'] == 'Pass']
...
events_pn_STO['pass_maker'] = events_pn_STO['player']
events_pn_STO['pass_receiver'] = events_pn_STO['player'].shift(-1)

print(events_pn_LEI.head(10).to_markdown())
print(events_pn_STO.head(10).to_markdown())
...
#Substitutes
substitution_STO = events_STO[events_STO['type'] == 'Substitution']
...
substitution_STO_minute = np.min(substitution_STO['minute'])
substitution_STO_minute_data = substitution_STO[substitution_STO['minute'] == substitution_STO_minute]
substitution_STO_second = np.min(substitution_STO_minute_data['second'])
print("minute =", substitution_STO_minute, "second =",  substitution_STO_second)

events_pn_STO = events_pn_STO[(events_pn_STO['minute'] <= substitution_STO_minute)]
...
events_pn_LEI = events_pn_LEI[['index','minute', 'second', 'team', 'type', 'pass_outcome', 'player', 'pass_maker', 'pass_receiver', 'pass_maker_x', 'pass_maker_y', 'pass_receiver_x', 'pass_receiver_y','pass_length','play_pattern','pass_through_ball']]

print('hello\n',events_pn_LEI.head(10).to_markdown(),'\n',len(events_pn_LEI2))
print(events_pn_LEI2.to_markdown(),file=sourceFile2)
...
for index, row in av_loc_LEI.iterrows():
    pitch.annotate(players_LEI[row.name], xy=(row.pass_maker_x, row.pass_maker_y), c='black', va='center', ha='center',
                   size=10, ax=ax)
plt.title("Pass network for Leicester against Stoke City", size = 20)
plt.savefig("LEI_VS_STO_PASS_GRAPH.png")
plt.show()
...
plt.savefig("LEI_VS_STO_WEIGHTED_PASS_GRAPH.png")
plt.show()
...
def player_degree():
    ...
    plt.savefig("LEI_VS_STO_DEGREE.png")
    plt.show()
    ...
    plt.savefig("LEI_VS_STO_INDEGREE.png")
    plt.show()
    ...
    plt.savefig("LEI_VS_STO_OUTDEGREE.png")
    plt.show()
def modified_graph():
    ...
    plt.title("Modified pass network for Leicester vs Stoke", size = 20)
    plt.savefig("LEI_VS_STO_MODIFIED.png")
    plt.show()
...
def goal():
    print('\nGoals\n--------------------------------',file=sourceFile)
    events_pn = LEI_STO_events[['minute', 'second', 'team', 'type', 'location','shot_end_location','player','shot_outcome']]
    ...
    plt.savefig("LEI_VS_STO_GOAL.png")
    plt.show()
...
plt.title("Adjacency matrix for Leicester vs Stoke City pass network")
plt.show()
player_degree()
SpectralClustering.spectral(A_LEI,G_LEI,"LEI_VS_STO_SPECTRAL")
goal()
