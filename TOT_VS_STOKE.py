...
sourceFile = open('TOT_V_STOKE.txt', 'w')
print('Tottenham VS Stoke City\n--------------------------------',file=sourceFile)

#print(match.to_markdown())

TOT_STO_events=sb.events(match_id=3754270)
for i in range(380):
    if match['match_id'][i] == 3754270:
        print(match['home_team'][i], ':', match['home_score'][i], '-', match['away_team'][i], ':', match['away_score'][i],file=sourceFile)

print(TOT_STO_events.head(10).to_markdown())

tact = TOT_STO_events[TOT_STO_events['tactics'].isnull() == False]
...
#Events for TOT & STOTED

events_pn = TOT_STO_events[['minute', 'second', 'team', 'type', 'location', 'pass_end_location', 'pass_outcome', 'player','pass_length']]
#print(TOT_STO_events.columns)
...
for index, row in av_loc_TOT.iterrows():
    pitch.annotate(players_TOT[row.name], xy=(row.pass_maker_x, row.pass_maker_y), c='black', va='center', ha='center',
                   size=10, ax=ax)
plt.title("Pass network for Tottenham against Stoke City", size = 20)
plt.savefig("TOT_VS_STOKE_PASS_GRAPH.png")
plt.show()
...
plt.title("Pass network for Tottenham vs Stoke City", size = 20)
plt.savefig("TOT_VS_STOKE_PASS_WEIGHTED.png")
plt.show()
...
def player_degree():
    ...
    plt.xlabel("degree")
    plt.title("Player pass degrees for Tottenham VS Stoke ", size=16)
    plt.savefig('TOT_VS_STOKE_PASS_DEGREE.png')
    plt.show()
    ...
    plt.ylabel("Player Jersey number")
    plt.xlabel("indegree")
    plt.title("Player pass indegrees for Tottenham vs Stoke", size=16)
    plt.savefig('TOT_VS_STOKE_PASS_INDEGREE.png')
    plt.show()
    ...
    plt.title("Player pass outdegrees for Tottenham vs Stoke City", size=16)
    plt.savefig('TOT_VS_STOKE_PASS_OUTDEGREE.png')
    plt.show()
def modified_graph():
    ...
    plt.title("Modified pass network for Tottenham vs Stoke", size = 20)
    plt.savefig('TOT_VS_STOKE_MODIFIED_GRAPH.png')
    plt.show()
...
def goal():
    print('\nGoals\n--------------------------------', file=sourceFile)
    events_pn = TOT_STO_events[['minute', 'second', 'team', 'type', 'location','shot_end_location','player','shot_outcome']]
    ...
    plt.savefig('TOT_VS_STOKE_SHOT_MAP.png')
    plt.show()
#adjacency matrix
